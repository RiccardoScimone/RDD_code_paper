...
nfolds = 10
set.seed("18011996")
folds = sample(1:nfolds,size = 2311, replace = TRUE)
table(folds)
B = 360
K = 16
...
for ( f in 1:nfolds){
  to_use = which(folds != f)
  cl = makePSOCKcluster(12)
  registerDoParallel(cl)
  ret_t = try(x,silent = T)
  while(class(ret_t) == "try-error")
    ret_t = try(foreach (i=1:B,.packages =  c("RGeostats","radiant.data")) %dopar% {
      ...
      mat_param = t(matrix(NA,N_obs,6))
      for ( j in 1:K)
      { 
        idxs_obs = which(assignment_obs[to_use] == j)
        idxs_obs_global = which(assignment_obs == j)
        db = db.create(spatial_data[to_use,][idxs_obs,],flag.grid = F,ndim = 2,autoname = F)
        dirs = seq(0,150, by = 30)
        vario = vario.calc(db,dir = dirs)
        fit = model.auto(vario,struct = c("K-Bessel"),param = c("P=2","A=60"),lower = c("P=2","A = 10"),upper = c("P=2","A = 90"),verbose = 0) 
        mat_param[2:6,idxs_obs_global] = unlist(get_param(fit[1]))
        mat_param[1,idxs_obs_global] = mean(spatial_data[idxs_obs,3])
      }
      print(c(f,i))
      sink()
      t(mat_param)
...
RDD_estimate = list()
for ( i in 1:f){
  ret[[i]] = simplify2array(ret[[f]])
  RDD_estimate[[i]] = extract_spatial_predictions_median(ret[[f]])
}
save(RDD_estimate,folds, file = paste0(dir_path,"/RDD_fitting_Crossvalid",K,".Rdata"))
