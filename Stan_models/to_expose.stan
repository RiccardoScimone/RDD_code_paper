functions {
     ...
     matrix matern_ns_corr_mat(matrix Locations, matrix[] Aniso, vector dets, int  nu, vector sigma)
  { 
    ...
    vector matern_ns_corr_vec(vector ref_loc, matrix Locations, matrix ref_Aniso, matrix[] Aniso, real ref_det, vector dets, int nu, real ref_sigma, vector sigma )
  {
    ...
    for ( j in 1:N)
      { 
        ...
        v[j] = ref_sigma*sigma[j]*sqrsqrdet*sqrsqrdets[j] * normdet * norm_const * Q^nu * modified_bessel_second_kind(nu,Q);
      }
        return (v);
  }
