...
model {
  ...
  if (fit_mu)
    if (fit_sigma)
      Y ~ nngp(X_mu[1]*beta_mu[1], exp(X_sigma[1]*beta_sigma[1]),Locations,Aniso,Dets,NN_ind,N,M,nu);
    else
      Y ~ nngp(X_mu[1]*beta_mu[1], rep_vector(1,N),Locations,Aniso,Dets,NN_ind,N,M,nu);
  else
    if (fit_sigma)
      Y ~ nngp(rep_vector(0,N), exp(X_sigma[1]*beta_sigma[1]),Locations,Aniso,Dets,NN_ind,N,M,nu);
    else
      Y ~ nngp(rep_vector(0,N), rep_vector(1,N),Locations,Aniso,Dets,NN_ind,N,M,nu);
}
